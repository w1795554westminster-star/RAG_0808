...
def call_gemini_cross_check(doc_a_info: dict, doc_b_info: dict, domain: str) -> dict:
    """
    Sends Evidence Package (Doc A & Doc B) to Gemini 3.6 Flash for Cross-Comparison analysis.
...
"""
    ...
    if not GEMINI_API_KEY:
        # Structured mock fallback if API key is missing
...
class AIComplianceChecker:
    ...
    def run_compliance_check_suite(self) -> list:
        ...
        for pair in test_pairs:
            ...
            # Pick representative chunks for comparison
            chunk_a = sub_a.iloc[1 if len(sub_a) > 1 else 0].to_dict()
            chunk_b = sub_b.iloc[1 if len(sub_b) > 1 else 0].to_dict()

            # Execute LLM Cross-Check
            analysis = call_gemini_cross_check(chunk_a, chunk_b, domain)
