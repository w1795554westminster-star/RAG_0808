...
dotenv.load_dotenv(os.path.join(base_dir, "buoi_14", ".env"))
...
class ComplianceGapChecker:
    """
    AI Compliance Gap Checker for Buổi 17.
    Compares External Requirements (e.g. NHNN Circulars) vs Internal Policies.
    Enforces Strict Audit & Governance Rules:
    - Never concludes compliance based on similarity score alone.
    - Never assigns DAP_UNG without real internal evidence.
    - Never assigns THIEU solely because retriever found no match.
    - If corpus lacks INTERNAL_POLICY, flags DATA GAP without hallucinating conclusions.
    ...
    def __init__(self, csv_path: str = None):
        ...
        # Audit internal vs external count in corpus based on strict evidence
        self.internal_docs = []
        self.external_docs = []
        ...
        for did, group in self.df_corpus.groupby('document_id'):
            ...
            # Strict classification: State agencies, Laws, Circulars, Decrees are External
    ...
    def analyze_gap(self, external_requirement_chunk: dict, user_role: str = "Risk_Manager") -> dict:
        """
        Analyzes a single external requirement chunk against internal policies.
        """
        ...
        # If internal policies are missing in corpus, return CHUA_DU_BANG_CHUNG due to DATA GAP
        ...
        # Search for internal policy candidates using Adapter
        ...
        # Filter candidates to keep only internal docs
        ...
        record = {
            ...
            "classification": "DAP_UNG" if top_match.get("score", 0) > 3.0 else "CHENH_LECH",
            "reason": "Phát hiện bằng chứng quy định nội bộ đối ứng.",
            "confidence": 0.85,
            "review_status": "NEEDS_HUMAN_REVIEW",
            "request_id": req_id
        }
...
def export_initial_gap_results_csv():
    ...
    if not status_info["is_data_ready"]:
        # Add a clear DATA GAP status row in CSV if data is insufficient
        sample_gap = {
