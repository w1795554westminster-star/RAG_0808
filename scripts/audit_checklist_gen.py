...
def generate_checklist_with_llm(domain: str, unit: str, context_chunks: list) -> list:
    """
    Calls configured LLM Provider (Ollama or Gemini) to generate audit checklist items.
    """
    context_text = ""
...
"""
    ...
    if provider == "ollama":
        print(f"[AuditChecklistGen] Using LLM_PROVIDER=ollama (Model: {os.getenv('OLLAMA_MODEL', 'qwen3:0.6b')})")
        client = OllamaClient()
        res = client.generate(prompt, format_json=True, temperature=0.2)
        if isinstance(res, list):
            return res
        elif isinstance(res, dict):
            if "checklist" in res and isinstance(res["checklist"], list):
    ...
    # Gemini Provider
    print(f"[AuditChecklistGen] Using LLM_PROVIDER=gemini (Model: {MODEL_NAME})")
